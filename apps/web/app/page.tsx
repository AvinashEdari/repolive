...
export default function Home() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = (await response.json()) as { files?: unknown[]; detail?: string };
      if (!response.ok) throw new Error(result.detail ?? "This repository could not be validated.");
      setState({ kind: "success", message: `Repository ingested: ${result.files?.length ?? 0} files discovered.` });
